import { prop_rent } from './objetos.js';
...
let recorrer = () => {
    for (let i = 0; i < 8; i++) { 
        let propiedad = Object.keys(prop_rent[i]); /* creo los array con las keys */
        ...
        let getNombre = prop_rent[i][propiedad.find(key => key === 'nombre')];
        let getSrc = prop_rent[i][propiedad.find(key => key === 'src')];
        let getDescripcion = prop_rent[i][propiedad.find(key => key === 'descripcion')];
        let getUbicacion = prop_rent[i][propiedad.find(key => key === 'ubicacion')];
        let getHabitaciones = prop_rent[i][propiedad.find(key => key === 'habitaciones')];
        let getCosto = prop_rent[i][propiedad.find(key => key === 'costo')];
        let getSmoke = prop_rent[i][propiedad.find(key => key === 'smoke')];
        let getPets = prop_rent[i][propiedad.find(key => key === 'pets')];
        let getParking = prop_rent[i][propiedad.find(key => key === 'parking')];
        let getBanos = prop_rent[i][propiedad.find(key => key === 'banos')];
